...
def Fetch() :
	...
	FulearnCur.execute(textwrap.dedent('''
		SELECT `machine`,`sn`,`block_status`,`component`,`test_type`,`status`,`end_time`,`seq` FROM `analog_test`
	...
	for row in FulearnCur :
		...
		findRetest.execute(textwrap.dedent('''
			SELECT * FROM `analog_test` WHERE `sn` = '{0}' AND `component` = '{1}' AND end_time = '{2}' AND seq > '{3}' AND test_type = '{4}' 
			ORDER BY `seq` ASC
			...
			if Retest_Pass is True:
				...
				if has_record is False :
					...
					findRetest_again.execute(textwrap.dedent('''
						SELECT * FROM `analog_test` WHERE `sn` = '{0}' AND `component` = '{1}' AND end_time > '{2}' AND test_type = '{3}' 
		...
		SqlList.append(textwrap.dedent('''
			UPDATE analog_test SET sfc_repair = '{0}',label = '{1}' WHERE sn = '{2}' AND component = '{3}' AND test_type = '{4}';
	...
	for CPK_twice in stored_component_new:
		...
		# print('CPK = ' + str(CPK) + '\n')
		SqlList.append(textwrap.dedent('''
			UPDATE analog_test SET cpk = '{0}' WHERE component = '{1}' AND test_type = '{2}';
			'''.format(CPK,sp[0],sp[1])))
		# Cur = commonObj.MySqlConn.cursor()
		# Cur.execute(textwrap.dedent('''
		# 	UPDATE ICT_Project_V2.analog_test SET cpk = '{0}' WHERE component = '{1}' AND test_type = '{2}';
		# 	'''.format(CPK,sp[0],sp[1])))
		# commonObj.MySqlConn.commit()
		# Cur.close()
		countCPK.close()
