...
# 根據下的參數轉換執行環境
Env = {}
Env['ICT'] = ['ICT','analog_powered_result','73-18275-04','label_18275']
Env['ICT4'] = ['ICT','analog_powered_result','73-18274-04','label_18274']
Env['ICT_Exp'] = ['ICT_exp','analog_powered_result','73-18275-04','label_18275']
TestDB = ''
TestTB = ''
TestBoard = ''
if len(sys.argv) > 1 :
...
def Fetch() :
	...
	FulearnCur.execute(textwrap.dedent('''
		...
		INNER JOIN ict_result b ON a.machine=b.machine AND a.sn=b.sn AND a.end_time=b.end_time AND a.status = 1 WHERE b.board='{1}' 
		...
		if end_time > (now - day90):
			...
			if sameSymptom is False or isNDF is True:
				...
				findRetest.execute(textwrap.dedent('''
					...
					WHERE b.board='{1}' AND a.`sn` = '{2}' AND a.`component` = '{3}' AND a.`test_condition` = '{4}' AND a.end_time > '{5}'
					GROUP BY `end_time` ORDER BY `end_time` ASC
					'''.format(TestTB,TestBoard,sn,component,test_condition,end_time)))
				isDone = False		#邏輯判斷結束
				re_sn = ''
				re_time = ''
				if findRetest.rowcount == 0 :   #沒有重測 or SQL query錯誤
					...
					if Retest_Pass is True:
						#計算CPK
						CPK = 0
						component_combine = component+'|'+test_condition
						if component_combine in stored_CPK:
							...
							countCPK.execute(textwrap.dedent('''
								SELECT * FROM {0} a
								INNER JOIN ict_result b ON a.machine=b.machine AND a.sn=b.sn AND a.end_time=b.end_time 
								WHERE b.board='{1}' AND a.component = '{2}' AND a.test_condition = '{3}' ORDER BY a.`end_time` ASC
								'''.format(TestTB,TestBoard,component,test_condition)))
							# t2 = time.time()
							...
							for data in countCPK:
								if (data[5] >= 0 and data[5] <= ((data[9]+data[10])/2)*5):		#篩選掉明顯有問題的資料
									if (data[9]-data[10]) != 0:
			...
			SqlList.append(textwrap.dedent('''
				INSERT IGNORE INTO {0} (logic_type,seq,sn,test_condition,label,label_no) VALUES ('analog powered','{1}','{2}','{3}','{4}','{5}');
				'''.format(LabelTB,seq,sn,test_condition,label,label_no)))
	...
	for CPK_twice in stored_component_new:
		...
		SqlList.append(textwrap.dedent('''
			UPDATE IGNORE {0} SET cpk = '{1}' WHERE component ='{2}' AND test_condition = '{3}';
	...
	# 	'''.format(LabelTB)))
	truncate.execute(textwrap.dedent('''
		DELETE FROM {0} WHERE logic_type = 'analog powered'
		'''.format(LabelTB)))
	truncate.close()
