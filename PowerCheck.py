...
def Fetch() :
	...
	for row in FulearnCur :
		...
		if end_time > (now - day90):
			...
			if sameSymptom is False or isNDF is True:
				...
				if findRetest.rowcount == 0 :   #沒有重測 or SQL query錯誤
					...
					if Retest_Pass is True:
						#計算CPK
						CPK = 0
						pwr_check_combine = power_check_type+'|'+power_check
			...
			SqlList.append(textwrap.dedent('''
				INSERT IGNORE INTO {0} (logic_type,seq,sn,power_check_type,power_check,label,label_no) VALUES ('powercheck','{1}','{2}','{3}','{4}','{5}',{6});
				'''.format(LabelTB,seq,sn,power_check_type,power_check,label,label_no)))
	...
	# 	'''.format(LabelTB)))
	truncate.execute(textwrap.dedent('''
		DELETE FROM {0} WHERE logic_type = 'powercheck'
		'''.format(LabelTB)))
	truncate.close()
