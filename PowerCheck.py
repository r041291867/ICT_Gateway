...
def Fetch() :
	...
	FulearnCur.execute(textwrap.dedent('''
		SELECT a.*,b.mfg FROM power_on_result a
	...
	for row in FulearnCur :
		...
		isNDF = False 			#是否NPF
		debugRow = sn +' '+ power_check_type +' '+ power_check +': '			#追蹤流程走向
		...
		if end_time > (now - day90):
			...
			for fail_info in SFC_fail['data'] :
				...
				#判定Fail Symptom是否一致
				if fail_info['Repair']['FailSymptom'] == 'failed power supply test' \
			...
			#找不到fail紀錄 or FailSymptom不一致 or NPF
				...
				findRetest.execute(textwrap.dedent('''
					...
					WHERE board='{0}' AND `sn` = '{1}' AND `end_time` > '{2}' AND `power_check_type` = '{3}'
					GROUP BY `end_time` ORDER BY `end_time` ASC
					'''.format(board,sn,end_time,power_check_type)))
				isDone = False		#邏輯判斷結束
				re_sn = ''
				re_time = ''
				if findRetest.rowcount == 0 :   #沒有重測 or SQL query錯誤
					...
					if Retest_Pass is True:
						#計算CPK
						CPK = 0
						pwr_check_combine = power_check_type+'|'+power_check+'|'+board
						if pwr_check_combine in stored_CPK:
							CPK = stored_CPK[pwr_check_combine]
							if CPK > 0.67 : 
								stored_sn.append(sn)
							...
							countCPK.execute(textwrap.dedent('''
								SELECT * FROM power_on_result
								WHERE board='{0}' AND power_check_type = '{1}' AND power_check = '{2}' 
								AND end_time BETWEEN '{3}' AND '{4}'
								ORDER BY `end_time` ASC
								'''.format(board,power_check_type,power_check,CPK_90day,EndTime)))
							# t2 = time.time()
							...
							CP = T[0]/(VAR*6)
							CPK = (1-CA)*(CP)
							stored_CPK[pwr_check_combine] = CPK
							isDone = True
							# t3 = time.time()
	...
	for CPK_twice in stored_component_new:
		...
		countCPK.execute(textwrap.dedent('''
			SELECT * FROM power_on_result
			WHERE board='{0}' AND power_check_type = '{1}' AND power_check = '{2}' AND end_time BETWEEN '{3}' AND '{4}'
			ORDER BY `end_time` ASC
		...
		SqlList.append(textwrap.dedent('''
			UPDATE label SET cpk = '{0}' WHERE power_check_type = '{1}' AND power_check = '{2}' AND board = '{3}';
