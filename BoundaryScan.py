...
def Fetch() :
	...
	FulearnCur.execute(textwrap.dedent('''
		...
		INNER JOIN ict_result b ON a.machine=b.machine AND a.sn=b.sn AND a.end_time=b.end_time AND a.status != 0 WHERE b.board='{0}' 
	...
	for row in FulearnCur :
		...
		if end_time > (now - day90):
						...
						if findTestjet.rowcount == 0:
							...
							if component in stored_LL:
								...
								countTotal.execute(textwrap.dedent('''
									SELECT a.*,b.`board` FROM `boundary_scan_result` a 
									INNER JOIN ict_result b ON a.machine=b.machine AND a.sn=b.sn AND a.end_time=b.end_time 
									WHERE b.board='{0}' AND a.component = '{1}'
								...
								countFail.execute(textwrap.dedent('''
									SELECT a.*,b.`board` FROM `boundary_scan_result` a 
									INNER JOIN ict_result b ON a.machine=b.machine AND a.sn=b.sn AND a.end_time=b.end_time 
									WHERE b.board='{0}' AND a.component = '{1}' AND a.status != 0
									Group by end_time ORDER BY `end_time`,`seq` ASC
									'''.format(TestBoard,component)))
			...
			SqlList.append(textwrap.dedent('''
				INSERT IGNORE INTO {0} (logic_type,seq,sn,label,label_no) VALUES ('boundary scan','{1}','{2}','{3}','{4}');
	...
	# 	'''.format(LabelTB)))
	truncate.execute(textwrap.dedent('''
		DELETE FROM {0} WHERE logic_type = 'boundary scan'
		'''.format(LabelTB)))
	truncate.close()
